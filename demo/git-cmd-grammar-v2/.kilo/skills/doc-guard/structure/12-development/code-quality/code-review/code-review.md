...
3.	Reviewer makes code reviews
	- He marks bad or suspicious code with ```//CODEREVIEW:``` comment
	- Or directly fixes the code
	- Commit with ```[CODEREVIEW]``` commit message
4.	QA personnel monitors source control for reviewed code
	- Find all files with ```//CODEREVIEW```
...
5.	Developer fixes the code, based on code review:
	...
	- commit with ```[FIX][CODEREVIEW]``` and description of changes 
...
- Place ``` //REVIEW:  message for reviewer ``` into your source code, if you need review of your code. Make message clear and short
- Commit your source code to development branch
- Reviewers will be notified
- You will receive answer with ```//CODEREVIEW:``` comments in your code and receive email
...
- Find and review committed files with ```[FIX]``` to review reasons for bugs and review fix code quality
- Other…

### Reviewer makes code review

- Place ``` //CODEREVIEW: message for developer ``` comments in the source code
- Use ``` //COREREVIEW: CRV XXXX ``` format if developer violates concrete rule from coding-rules
...
- Monitor notifications from QA and review ``` [FIX][CODEREVIEW] ``` commits to see how developers fixed reported issues
...
- Commit with ```[FIX][CODEREVIEW]``` commit message and shortly explain principle of fixes
