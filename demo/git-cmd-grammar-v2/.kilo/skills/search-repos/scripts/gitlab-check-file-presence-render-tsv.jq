# gitlab-check-file-presence-render-tsv.jq
# Render gitlab-check-file-presence JSON output as a TSV table.
#
# Usage: jq -rf .roo/skills/search-repos/gitlab-check-file-presence-render-tsv.jq /tmp/all.json
#
...
(
  ...
  | map(
      . as $row |
      ([$row.server, $row.project_path] + [$cols[] | $row.paths[.] | render_state]) | join("\t")
    )
) |
join("\n")
