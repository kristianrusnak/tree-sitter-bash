...
#### Follow-up experiment (post-ship): a genuinely single shared prefix symbol — still corrupts

A later investigation revisited option (a)'s premise. The original diagnosis blamed *duplication*: "two structurally-identical top-level productions" each independently inlining their own `repeat(choice(variable_assignment, redirect))`, producing two distinct generated repeat symbols (`command_repeat1`, `git_command_repeat1`) even when both call sites shared one JS helper function — because tree-sitter never deduplicates an inline `repeat(...)`/`seq(...)`/`choice(...)` expression across call sites, only a genuine separate rule reference is a single symbol.

That distinction was tested directly: a new, *actually* non-duplicated `_command_prefix` rule —

```js
_command_prefix: $ => repeat1(choice(
  $.variable_assignment,
  field('redirect', $._redirect),
)),
```

— defined once and referenced by symbol (`optional($._command_prefix)`) from both `command` and `git_command`, instead of each inlining its own `repeat(...)`. This is structurally different from the earlier attempt: there is exactly one `_command_prefix_repeat1` in the generated grammar, referenced twice, not two separate generated repeats. `tree-sitter generate` needed the same shape of conflict entries as before (`[$.command, $.git_command, $.variable_assignments]`, `[$.redirected_statement, $.git_command, $.command]`) and produced them cleanly.

Result: `VAR=x git status` and `2>&1 git status` now correctly parsed as `git_command` with a leading `assignment`/`redirect`, as hoped — **but the exact same newline-corruption bug reappeared**, byte-for-byte identical to before: `VAR1=x\nVAR2=y echo` still lexed `\nVAR2=y` into the `command_name` token as one bogus 7-character word, confirmed again via `tree-sitter parse --debug` (`lex_internal state:684` swallowing `10 'V' 'A' 'R' '2' '=' 'y' 10` into `sym:word, size:7`, the same lexer state number as the original attempt).

A follow-up controlled A/B test pinned the cause more precisely than the original write-up: with the shared `_command_prefix` rule's code left in place but `git_command` removed from `_statement_not_subshell`/`_statement_not_pipeline` (made unreachable, everything else unchanged), `VAR1=x\nVAR2=y echo` parses correctly again — two separate statements, no corruption. Put back, it corrupts again. So the corruption is **not** actually caused by "how many distinct generated repeat symbols exist" (the thing the original diagnosis named and the thing this experiment specifically eliminated) — it's caused by `git_command` merely being *reachable* as a second interpretation after `_command_prefix`/`variable_assignment`, regardless of whether the prefix machinery is duplicated or genuinely shared. Something about widening the follow-set after a variable_assignment (a git-shaped name is now also a valid continuation, not just a plain `command_name`) causes tree-sitter's generated *internal* lexer table (`lex_internal`, not `scanner.c`) to compute a wrong transition at the post-assignment-value state, independent of the repeat-symbol-sharing angle.

This means: **no purely grammar-level (`grammar.js`) restructuring closes this gap.** Both the "duplicate the prefix" and "genuinely share one prefix symbol" shapes hit the identical wall, and the wall sits inside `lex_internal` — generated code, not something `grammar.js` or `scanner.c` can influence except by changing what's *reachable*, which is exactly what triggers it. Reducing option (a) to something concrete: since `lex_internal` corrupts but `scanner.c`'s hand-written scanner already owns the `/\n/` symbol (it's declared in `externals`), a fix would have to make the external scanner claim the newline explicitly in this context, ahead of the generated DFA — an actual `scanner.c` change, not a `grammar.js` one, and one with repo-wide blast radius since newline handling is shared by every construct in the grammar, not just git. That path was considered and explicitly **not** pursued (scanner.c changes are out of scope for this fork's distribution constraints — it ships as a `.wasm` build, and modifying the hand-written scanner was ruled out for that reason). The documented limitation therefore stands as shipped: `git_command` gets full option/subcommand structure only when it's the first token of the statement; a leading `VAR=x` or I/O redirect falls back to plain `command`, with git's own tokens landing as undifferentiated arguments inside it.
