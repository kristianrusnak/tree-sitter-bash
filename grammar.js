...
// Grammar extensions live under grammars/ (one folder per extension), keeping
// the upstream bash grammar intact for conflict-free upstream merges.
// Wiring contract and per-extension layout: grammars/git/git-grammar.js.
// Modified by AI Kilo Code, used model gti-litellm/glm-5.3-flash.
const gitGrammar = require('./grammars/git/git-grammar');
...
module.exports = grammar({
  ...
  rules: {
    ...
    // Grammar extensions (see grammars/) spread their rule definitions into
    // the map at this exact position — after `unset_command`, before
    // `command` — so the generated files (src/grammar.json, src/parser.c)
    // keep their rule/symbol ordering stable across refactors.
    ...gitGrammar(SPECIAL_CHARACTERS, noneOf),
